...
if (!empty($produtos_selecionados)) {
    // Você pode usar esta lista de IDs para salvar um pedido no banco de dados
    // Por exemplo, você pode criar uma tabela 'pedidos' e outra 'itens_pedido'
    
    // Exemplo de como você poderia salvar o pedido
    try {
        ...
        $id_cliente = 1; 
        $sql_pedido = "INSERT INTO pedidos (id_cliente, data_pedido, status) VALUES (?, NOW(), 'Pendente')";
        $stmt_pedido = $conn->prepare($sql_pedido);
        $stmt_pedido->execute([$id_cliente]);
        $id_pedido = $conn->lastInsertId();
        
        // 2. Inserir os itens do pedido na tabela 'itens_pedido'
        $sql_item = "INSERT INTO itens_pedido (id_pedido, id_produto, quantidade, preco_unitario) VALUES (?, ?, 1, ?)";
        ...
        foreach ($produtos_selecionados as $id_produto) {
            ...
            if ($preco_unitario) {
                $stmt_item->execute([$id_pedido, $id_produto, $preco_unitario]);
            }
        }
        ...
        // Opcional: Limpar o carrinho após a finalização do pedido
        $_SESSION['carrinho'] = [];
